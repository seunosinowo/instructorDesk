import React from 'react';
import { 
  Award, Users, Calendar, Globe, Phone, 
  BookOpen, CheckCircle, MapPin, ExternalLink,
  GraduationCap, Building, Bookmark,
  BarChart2, Shield, Library, Activity
} from 'lucide-react';
import { motion } from 'framer-motion';

interface SchoolProfileViewProps {
  profile: any;
}

const SchoolProfileView: React.FC<SchoolProfileViewProps> = ({ profile }) => {
  if (!profile) return null;

  // School Information Card
  const SchoolInfoCard = () => (
    <motion.div 
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ delay: 0.1 }}
      className="bg-white rounded-2xl shadow-lg p-6 border border-orange-100"
    >
      <div className="flex items-center mb-6">
        <Building className="text-orange-500 mr-3" size={24} />
        <h3 className="text-xl font-bold text-gray-800">School Information</h3>
      </div>
      
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-4">
          <div className="flex items-center">
            <div className="bg-orange-100 p-2 rounded-lg mr-4">
              <BookOpen className="text-orange-600" size={18} />
            </div>
            <div>
              <p className="text-sm text-gray-500">School Name</p>
              <p className="font-medium text-gray-800">{profile.schoolName}</p>
            </div>
          </div>
          
          <div className="flex items-center">
            <div className="bg-orange-100 p-2 rounded-lg mr-4">
              <Shield className="text-orange-600" size={18} />
            </div>
            <div>
              <p className="text-sm text-gray-500">School Type</p>
              <p className="font-medium text-gray-800 capitalize">{profile.schoolType}</p>
            </div>
          </div>
          
          <div className="flex items-center">
            <div className="bg-orange-100 p-2 rounded-lg mr-4">
              <MapPin className="text-orange-600" size={18} />
            </div>
            <div>
              <p className="text-sm text-gray-500">Address</p>
              <p className="font-medium text-gray-800">{profile.address}, {profile.city}, {profile.state}</p>
            </div>
          </div>
          
          <div className="flex items-center">
            <div className="bg-orange-100 p-2 rounded-lg mr-4">
              <Phone className="text-orange-600" size={18} />
            </div>
            <div>
              <p className="text-sm text-gray-500">Phone</p>
              <p className="font-medium text-gray-800">{profile.phoneNumber || 'Not specified'}</p>
            </div>
          </div>
        </div>
        
        <div className="space-y-4">
          <div className="flex items-center">
            <div className="bg-orange-100 p-2 rounded-lg mr-4">
              <Globe className="text-orange-600" size={18} />
            </div>
            <div>
              <p className="text-sm text-gray-500">Website</p>
              {profile.website ? (
                <a 
                  href={profile.website} 
                  target="_blank" 
                  rel="noopener noreferrer"
                  className="font-medium text-orange-600 hover:underline flex items-center"
                >
                  {profile.website} <ExternalLink size={14} className="ml-1" />
                </a>
              ) : (
                <p className="font-medium text-gray-800">Not specified</p>
              )}
            </div>
          </div>
          
          <div className="flex items-center">
            <div className="bg-orange-100 p-2 rounded-lg mr-4">
              <Award className="text-orange-600" size={18} />
            </div>
            <div>
              <p className="text-sm text-gray-500">Accreditation</p>
              <p className="font-medium text-gray-800">{profile.accreditations || 'Not specified'}</p>
            </div>
          </div>
          
          <div className="flex items-center">
            <div className="bg-orange-100 p-2 rounded-lg mr-4">
              <Calendar className="text-orange-600" size={18} />
            </div>
            <div>
              <p className="text-sm text-gray-500">Established</p>
              <p className="font-medium text-gray-800">{profile.establishedYear || 'Not specified'}</p>
            </div>
          </div>
        </div>
      </div>
    </motion.div>
  );

  // Statistics Card
  const StatisticsCard = () => (
    <motion.div 
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ delay: 0.2 }}
      className="bg-gradient-to-br from-orange-500 to-amber-500 rounded-2xl shadow-lg p-6"
    >
      <div className="flex items-center mb-6">
        <BarChart2 className="text-white mr-3" size={24} />
        <h3 className="text-xl font-bold text-white">School Statistics</h3>
      </div>
      
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white/20 p-4 rounded-xl text-center">
          <div className="text-white mb-2 flex justify-center">
            <Users size={24} />
          </div>
          <p className="text-sm text-white/80">Students</p>
          <p className="text-2xl font-bold text-white">{profile.studentCount || 0}</p>
        </div>
        
        <div className="bg-white/20 p-4 rounded-xl text-center">
          <div className="text-white mb-2 flex justify-center">
            <GraduationCap size={24} />
          </div>
          <p className="text-sm text-white/80">Teachers</p>
          <p className="text-2xl font-bold text-white">{profile.teacherCount || 0}</p>
        </div>
        
        <div className="bg-white/20 p-4 rounded-xl text-center">
          <div className="text-white mb-2 flex justify-center">
            <BookOpen size={24} />
          </div>
          <p className="text-sm text-white/80">Grade Levels</p>
          <p className="text-lg font-bold text-white">
            {profile.gradeLevels?.slice(0, 2).join(', ')}
            {profile.gradeLevels?.length > 2 ? '...' : ''}
          </p>
        </div>
        
        <div className="bg-white/20 p-4 rounded-xl text-center">
          <div className="text-white mb-2 flex justify-center">
            <Library size={24} />
          </div>
          <p className="text-sm text-white/80">Facilities</p>
          <p className="text-lg font-bold text-white">{profile.facilities?.length || 0}</p>
        </div>
      </div>
    </motion.div>
  );

  // Facilities Card
  const FacilitiesCard = () => (
    profile.facilities && profile.facilities.length > 0 && (
      <motion.div 
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.3 }}
        className="bg-white rounded-2xl shadow-lg p-6 border border-orange-100"
      >
        <div className="flex items-center mb-6">
          <Library className="text-orange-500 mr-3" size={24} />
          <h3 className="text-xl font-bold text-gray-800">Facilities</h3>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {profile.facilities.map((facility: string, index: number) => (
            <div key={index} className="flex items-center bg-orange-50 p-3 rounded-lg">
              <CheckCircle className="text-green-500 mr-3" size={16} />
              <span className="text-gray-800">{facility}</span>
            </div>
          ))}
        </div>
      </motion.div>
    )
  );

  // Activities Card
  const ActivitiesCard = () => (
    profile.extracurricularActivities && profile.extracurricularActivities.length > 0 && (
      <motion.div 
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.4 }}
        className="bg-white rounded-2xl shadow-lg p-6 border border-orange-100"
      >
        <div className="flex items-center mb-6">
          <Activity className="text-orange-500 mr-3" size={24} />
          <h3 className="text-xl font-bold text-gray-800">Extracurricular Activities</h3>
        </div>
        
        <div className="flex flex-wrap gap-2">
          {profile.extracurricularActivities.map((activity: string, index: number) => (
            <span 
              key={index}
              className="bg-orange-100 text-orange-800 px-3 py-1.5 rounded-full text-sm font-medium"
            >
              {activity}
            </span>
          ))}
        </div>
      </motion.div>
    )
  );

  // Description Card
  const DescriptionCard = () => (
    profile.description && (
      <motion.div 
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.5 }}
        className="bg-white rounded-2xl shadow-lg p-6 border border-orange-100"
      >
        <div className="flex items-center mb-6">
          <Bookmark className="text-orange-500 mr-3" size={24} />
          <h3 className="text-xl font-bold text-gray-800">About Our School</h3>
        </div>
        
        <p className="text-gray-700 leading-relaxed bg-orange-50 p-5 rounded-xl">
          {profile.description}
        </p>
      </motion.div>
    )
  );

  // Social Links Card
  const SocialLinksCard = () => (
    profile.socialLinks && Object.values(profile.socialLinks).some(val => val) && (
      <motion.div 
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.6 }}
        className="bg-white rounded-2xl shadow-lg p-6 border border-orange-100"
      >
        <div className="flex items-center mb-6">
          <Globe className="text-orange-500 mr-3" size={24} />
          <h3 className="text-xl font-bold text-gray-800">Connect With Us</h3>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {profile.socialLinks.facebook && (
            <a 
              href={profile.socialLinks.facebook}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center p-4 bg-orange-50 rounded-xl hover:bg-orange-100 transition-colors group"
            >
              <div className="bg-orange-600 p-2 rounded-lg mr-4">
                <svg className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path fillRule="evenodd" d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z" clipRule="evenodd" />
                </svg>
              </div>
              <div>
                <p className="font-medium text-gray-800">Facebook</p>
                <p className="text-sm text-gray-600">Follow us on Facebook</p>
              </div>
            </a>
          )}
          
          {profile.socialLinks.twitter && (
            <a 
              href={profile.socialLinks.twitter}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center p-4 bg-orange-50 rounded-xl hover:bg-orange-100 transition-colors group"
            >
              <div className="bg-orange-500 p-2 rounded-lg mr-4">
                <svg className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M8.29 20.251c7.547 0 11.675-6.253 11.675-11.675 0-.178 0-.355-.012-.53A8.348 8.348 0 0022 5.92a8.19 8.19 0 01-2.357.646 4.118 4.118 0 001.804-2.27 8.224 8.224 0 01-2.605.996 4.107 4.107 0 00-6.993 3.743 11.65 11.65 0 01-8.457-4.287 4.106 4.106 0 001.27 5.477A4.072 4.072 0 012.8 9.713v.052a4.105 4.105 0 003.292 4.022 4.095 4.095 0 01-1.853.07 4.108 4.108 0 003.834 2.85A8.233 8.233 0 012 18.407a11.616 11.616 0 006.29 1.84" />
                </svg>
              </div>
              <div>
                <p className="font-medium text-gray-800">Twitter</p>
                <p className="text-sm text-gray-600">Follow us on Twitter</p>
              </div>
            </a>
          )}
          
          {profile.socialLinks.instagram && (
            <a 
              href={profile.socialLinks.instagram}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center p-4 bg-orange-50 rounded-xl hover:bg-orange-100 transition-colors group"
            >
              <div className="bg-orange-600 p-2 rounded-lg mr-4">
                <svg className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path fillRule="evenodd" d="M12.315 2c2.43 0 2.784.013 3.808.06 1.064.049 1.791.218 2.427.465a4.902 4.902 0 011.772 1.153 4.902 4.902 0 011.153 1.772c.247.636.416 1.363.465 2.427.048 1.067.06 1.407.06 4.123v.08c0 2.643-.012 2.987-.06 4.043-.049 1.064-.218 1.791-.465 2.427a4.902 4.902 0 01-1.153 1.772 4.902 4.902 0 01-1.772 1.153c-.636.247-1.363.416-2.427.465-1.067.048-1.407.06-4.123.06h-.08c-2.643 0-2.987-.012-4.043-.06-1.064-.049-1.791-.218-2.427-.465a4.902 4.902 0 01-1.772-1.153 4.902 4.902 0 01-1.153-1.772c-.247-.636-.416-1.363-.465-2.427-.047-1.024-.06-1.379-.06-3.808v-.63c0-2.43.013-2.784.06-3.808.049-1.064.218-1.791.465-2.427a4.902 4.902 0 011.153-1.772A4.902 4.902 0 015.45 2.525c.636-.247 1.363-.416 2.427-.465C8.901 2.013 9.256 2 11.685 2h.63zm-.081 1.802h-.468c-2.456 0-2.784.011-3.807.058-.975.045-1.504.207-1.857.344-.467.182-.8.398-1.15.748-.35.35-.566.683-.748 1.15-.137.353-.3.882-.344 1.857-.047 1.023-.058 1.351-.058 3.807v.468c0 2.456.011 2.784.058 3.807.045.975.207 1.504.344 1.857.182.466.399.8.748 1.15.35.35.683.566 1.15.748.353.137.882.3 1.857.344 1.054.048 1.37.058 4.041.058h.08c2.597 0 2.917-.01 3.96-.058.976-.045 1.505-.207 1.858-.344.466-.182.8-.398 1.15-.748.35-.35.566-.683.748-1.15.137-.353.3-.882.344-1.857.048-1.055.058-1.37.058-4.041v-.08c0-2.597-.01-2.917-.058-3.96-.045-.976-.207-1.505-.344-1.858a3.097 3.097 0 00-.748-1.15 3.098 3.098 0 00-1.15-.748c-.353-.137-.882-.3-1.857-.344-1.023-.047-1.351-.058-3.807-.058zM12 6.865a5.135 5.135 0 110 10.27 5.135 5.135 0 010-10.27zm0 1.802a3.333 3.333 0 100 6.666 3.333 3.333 0 000-6.666zm5.338-3.205a1.2 1.2 0 110 2.4 1.2 1.2 0 010-2.4z" clipRule="evenodd" />
                </svg>
              </div>
              <div>
                <p className="font-medium text-gray-800">Instagram</p>
                <p className="text-sm text-gray-600">Follow us on Instagram</p>
              </div>
            </a>
          )}
          
          {profile.socialLinks.linkedin && (
            <a 
              href={profile.socialLinks.linkedin}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center p-4 bg-orange-50 rounded-xl hover:bg-orange-100 transition-colors group"
            >
              <div className="bg-orange-700 p-2 rounded-lg mr-4">
                <svg className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path fillRule="evenodd" d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" clipRule="evenodd" />
                </svg>
              </div>
              <div>
                <p className="font-medium text-gray-800">LinkedIn</p>
                <p className="text-sm text-gray-600">Follow us on LinkedIn</p>
              </div>
            </a>
          )}
        </div>
      </motion.div>
    )
  );

  return (
    <div className="space-y-8">
      <SchoolInfoCard />
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="space-y-8">
          <StatisticsCard />
          <SocialLinksCard />
        </div>
        <div className="space-y-8">
          <DescriptionCard />
          <FacilitiesCard />
          <ActivitiesCard />
        </div>
      </div>
    </div>
  );
};

export default SchoolProfileView;